# ELOI Studio — Mapa de Rotas

> Antes: `SITEMAP.md` na raiz. Movido para `docs/` em 2026-08-05.
...
| `/admin/dinheiro` | `telas/Dinheiro.tsx` | **Financeiro** em 5 abas: Movimentações (extrato do mês), A receber, A pagar (ambas ignoram o mês — dívida vencida é trabalho de hoje), Contas (saldo por conta, fatura e limite do cartão) e Recorrências (custo mensal e anual). Lançar receita/despesa/transferência, parcelar, **editar**, liquidar total ou parcial, **cancelar/reabrir (estorno)**, excluir, **pagar fatura do cartão** (vira transferência conta→cartão) e **pausar/retomar/encerrar recorrência**. |
| `/admin/projetos` | `telas/Projetos.tsx` | Funil por etapa calculada (orçamento → aprovado → execução → entregue → pago), no mês do seletor do cabeçalho (mesmo `mes` do store que Dinheiro; abre no atual; regra `mesDoProjeto`: competência → pagamento → prazo), o que não tem mês em seção própria; dentro, cliente e marca, com data na linha e marca de NF — sem nota, o ícone abre a folha de anexar (`FolhaNota` de Notas, já com serviço, cliente e valor). **Criar/editar serviço** e **aprovar proposta** (o trigger `trg_eloi_orcamento_aprovado` cria o serviço). |
| `/admin/clientes` · `/admin/clientes/:id` | `telas/Clientes.tsx` · `ClienteFicha.tsx` | Carteira com faturado/a receber/serviços por cliente; **cadastro e edição de cliente**. A ficha consolida cadastro, projetos, histórico financeiro, notas, **propostas e arquivos** do cliente. |
...
| `/admin/relatorios` | `telas/Relatorios.tsx` | Recorte por período/cliente/marca. Resultado de 12 meses, clientes e marcas (recebido, a receber, projetos, ticket), projetos por etapa, aging de recebíveis, fiscal, categorias, previsão em 3 cenários, metas. CSV por aba e impressão. |
...
| `/admin/*` (desconhecida) | `router.tsx` | Estado "página não encontrada" dentro do shell, com volta para a visão geral. Fora de `/admin`, redireciona para `/admin`. |
...
| `/painel-briefings/` | — | **Aposentado (2026-09-03)** — redirect 301 para `/admin/briefings`, que gera convite, lê a resposta, reabre e revoga. |
...
| `/painel-orcamentos/` | — | **Aposentado (2026-09-03)** — redirect 301 para `/admin/orcamentos`, que faz o ciclo inteiro da proposta. |
| `/orcamento-inteligente/` | — | **Aposentado** — redirect 301 para `/admin/orcamentos`. |
...
## Regras de domínio no banco (valem para todas as rotas)
- `/admin-app/` foi **removido em 2026-08-05** (SPA React 18 substituída por `app/`). `vercel.json` redireciona `/admin-app*` → `/admin` com 301, para não quebrar link antigo.
...
## Marca / Logo
- Logo oficial = wordmark em `assets/eloi-admin/wordmark.svg` (`viewBox 0 0 540.45 348`, **16 paths**, cor por `--logo-1`/`--logo-2` com fallback `#240043`/`#3c0a6b`).
- ⚠️ O desenho ainda letra **"ELOI Design Studio"**. O nome oficial passou a ser **ELOI Studio** em 2026-08-05; re-letrar o wordmark é trabalho de design, não de código. Todo texto (`alt`, `aria-label`, títulos, manifest) já diz "ELOI Studio".
...
- `docs/GLOSSARY.md` tem o vocabulário de domínio (Cliente/Projeto/Etapa/Decisão/etc.) usado pela lógica de negócio do painel admin.