...
| `/admin` e `/admin/*` | `app/dist/` (rewrite no `vercel.json`) | **SPA nova do rebuild** (Fundação, 2026-07-27) — Vite+React+TS em `app/`, dist COMMITADO (Vercel não builda). Hoje: login + shell com sidebar (5 áreas + Ferramentas) e as 7 rotas navegáveis; telas ainda placeholder, conteúdo real chega no sub-projeto 2. Sessão compartilhada com os painéis legados (mesmo `eloi_admin_token`). O hub estático antigo (`admin/index.html`) foi aposentado — no Vercel, arquivo estático ganha de rewrite, então ele PRECISOU sair do repo. Dashboard por período segue no legado `/gestao/` até o porte. |
