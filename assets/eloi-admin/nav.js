...
(function (w, d) {
  ...
  function styleTag(){
    return '<style id="eloi-nav-style">'
      ...
      + 'background:linear-gradient(180deg,rgba(16,0,43,.98),rgba(36,0,70,.96));border-right:1px solid #3C096C;'
      + 'backdrop-filter:blur(10px);display:flex;flex-direction:column;padding:22px 14px 16px;'
      + 'font-family:carbona-variable,system-ui,sans-serif;color:#fff;transition:transform .25s ease}'
      + '.eloi-nav .eloi-nav-logo{width:130px;margin:0 6px 22px;opacity:.95}'
      + '.eloi-nav .eloi-nav-logo svg{width:100%;height:auto;display:block}'
      + '.eloi-nav-sec{font-size:.62rem;letter-spacing:.09em;text-transform:uppercase;color:#9D4EDD;margin:16px 8px 6px;font-weight:600}'
      + '.eloi-nav-item{display:flex;align-items:center;gap:11px;padding:10px 12px;border-radius:10px;'
      + 'color:#E0AAFF;text-decoration:none;font-size:.9rem;line-height:1;transition:background .15s,color .15s}'
      + '.eloi-nav-item svg{flex:0 0 auto;opacity:.85}'
      + '.eloi-nav-item:hover{background:rgba(123,44,191,.22);color:#fff}'
      + '.eloi-nav-on{background:rgba(157,78,221,.28);color:#fff;box-shadow:inset 3px 0 0 #C77DFF}'
      + '.eloi-nav-on svg{opacity:1}'
      + '.eloi-nav-spacer{flex:1}'
      ...
      + 'background:rgba(60,9,108,.5);border:1px solid #3C096C;color:#E0AAFF;text-decoration:none;white-space:nowrap}'
      + '.eloi-nav-quick a:hover{border-color:#9D4EDD;color:#fff}'
      + '.eloi-nav-out{margin:0 4px;text-align:left;background:transparent;border:1px solid #3C096C;color:#C77DFF;'
      + 'font-family:inherit;font-size:.8rem;padding:9px 12px;border-radius:9px;cursor:pointer;transition:border-color .15s}'
      + '.eloi-nav-out:hover{border-color:#9D4EDD;color:#fff}'
      + '.eloi-nav-bar{position:fixed;top:0;left:0;right:0;height:52px;z-index:899;display:none;'
      + 'align-items:center;gap:12px;padding:0 14px;background:rgba(16,0,43,.96);border-bottom:1px solid #3C096C;'
      + 'backdrop-filter:blur(8px);font-family:carbona-variable,system-ui,sans-serif}'
      + '.eloi-nav-burger{background:none;border:none;color:#E0AAFF;cursor:pointer;padding:6px;display:flex}'
      + '.eloi-nav-bar .eloi-nav-logo{width:104px;margin:0}'
      + '.eloi-nav-scrim{position:fixed;inset:0;z-index:898;background:rgba(0,0,0,.5);display:none}'
